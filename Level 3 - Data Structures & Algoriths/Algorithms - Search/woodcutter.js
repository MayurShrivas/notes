...
It is an easy job for him since he has a nifty new woodcutting machine that can take down forests like wildfire. However, 
Ojas is only allowed to cut a single row of trees. Ojas's machine works as follows: Ojas sets a height parameter H (in metres), 
and the machine raises a giant sawblade to that height and cuts off all tree parts higher than H (of course, trees not higher than 
H meters remain intact). Ojas then takes the parts that were cut off. For example, if the tree row contains trees with heights of 
20, 15, 10, and 17 metres, and Ojas raises his sawblade to 15 metres, the remaining tree heights after cutting will be 15, 15, 10, 
and 15 metres, respectively, while Ojas will take 5 metres off the first tree and 2 metres off the fourth tree (7 metres of wood in total).
...
function woodCutting (A, B) {
  let min = 0;
  let max = Math.max(...A);
  
  while (min <= max) {
      let mid = parseInt((min + max) / 2);
      let amountOfWood = this.calculateWoodAmount (A, mid);
      
      if(amountOfWood === B) {
          return mid;  
      } else if(amountOfWood > B) {
          min = mid + 1;
      } else {
          max = mid - 1;
      }
...
function calculateWoodAmount (A, mid) {
let result = 0;
for(let tree of A) {
    if(tree > mid) {
        result += tree - mid;
...
console.log (calculateWoodAmount (A = [20, 15, 10, 17], B = 15));
